        ...
        <?php if (!empty($_SESSION['cart'])): ?>
            <div class="space-y-4">
                <?php foreach ($_SESSION['cart'] as $item): ?>
                    <div class="border rounded-lg p-4 flex justify-between items-center" id="cart-item-<?= $item[0]['id'] ?>">
                        <div>
                            <!-- <?php var_dump($item[0]) ?> -->
                            <h2 class="text-xl font-bold"><?= htmlspecialchars($item[0]['name']) ?></h2>
                            <p class="text-gray-600"><?= htmlspecialchars($item[0]['price']) ?>₽</p>
                        </div>
                        <div class="flex items-center space-x-2 gap-5">
                            <button class="btn-increase size-8 flex leading-[0] justify-center items-center rounded-full bg-gray-200 hover:bg-gray-400 active:bg-gray-600 transition text-2xl" data-product-id="<?= $item[0]['id'] ?>">+</button>
                            <p class="text-accent font-bold quantity-display" data-product-id="<?= $item[0]['id'] ?>"><?= $item[0]['quantity'] ?></p>
                            <button class="btn-decrease size-8 flex leading-[0] justify-center items-center rounded-full bg-gray-200 hover:bg-gray-400 active:bg-gray-600 transition text-2xl" data-product-id="<?= $item[0]['id'] ?>">-</button>
                            <button class="btn-remove !ml-5 size-8 flex leading-[0] justify-center items-center rounded-full bg-rose-500 hover:bg-rose-600 active:bg-rose-800 transition text-l" data-product-id="<?= $item[0]['id'] ?>">✕</button>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
