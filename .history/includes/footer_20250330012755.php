<footer class="bg-gray-900 text-white py-8">
    <div class="container mx-auto px-4 flex flex-col md:flex-row justify-between items-center">
        <!-- Левая часть: Название сайта -->
        <div class="text-center md:text-left mb-4 md:mb-0">
            <p class="text-lg font-bold">Coffee & Cozy</p>
            <p class="text-sm text-gray-400">Ваше уютное место для кофе и десертов</p>
        </div>

        <!-- Правая часть: Ссылки -->
        <div class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-6 text-center">
            <a href="/privacy-policy.php" class="text-gray-300 hover:text-white transition duration-300">Политика конфиденциальности</a>
            <a href="#" class="text-gray-300 hover:text-white transition duration-300">Контакты</a>
            <a href="#" class="text-gray-300 hover:text-white transition duration-300">О нас</a>
        </div>
    </div>

    <!-- Копирайт -->
    <div class="mt-6 border-t border-gray-800 pt-4 text-center text-sm text-gray-400">
        &copy; <?= date('Y') ?> Coffee & Cozy. Все права защищены.
    </div>
</footer>