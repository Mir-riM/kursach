                    ...
                    <?php foreach ($_SESSION['cart'] as $item): ?>
                        <div class="border rounded-lg p-4 flex justify-between items-center" id="cart-item-<?= $item['id'] ?>">
                            ...
                            <div class="flex items-center space-x-2 gap-5">
                                ...
                                <button class="btn-remove !ml-5 size-8 flex leading-[0] justify-center items-center rounded-full bg-rose-400 hover:bg-rose-600 active:bg-rose-800 transition text-l" data-product-id="<?= $item['id'] ?>">✕</button>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
                ...
                button.addEventListener('click', async (e) => {
                    ...
                    try {
                        ...
                        if (response.status === 200) {
                            ...
                            const quantityDisplay = Number(document.querySelector(`.quantity-display[data-product-id="${productId}"]`).innerText);
                            if (quantityDisplay) {
                                console.log(quantityDisplay.innerText)
                                const currentQuantity = Number(quantityDisplay);
