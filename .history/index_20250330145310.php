...
<article class="container mx-auto p-4 mt-10">
    ...
    <a href="./cart.php" class="count-cart w-full fixed bottom-5 z-50">
        <div
...
document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                if (cartTotalQuantityElement) {
                    ...
                    const cartBlock = document.querySelector('.fixed.bottom-5');
    ...
    document.querySelectorAll('.add-to-cart, .btn-increase, .btn-decrease, .btn-remove').forEach(button => {
        button.addEventListener('click', () => {
            setTimeout(() => {
                updateCartQuantity();
            }, 500); // Задержка для обработки изменений на сервере
        });
    });
});
</script>
